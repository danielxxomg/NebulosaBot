...
# ---------------------------------------------------------------------------
# cache-sync-realtime PR 2 — webhook config surface MUST be removed
# ---------------------------------------------------------------------------


class TestWebhookConfigRemoved:
    """cache-sync-realtime: the inbound webhook + Cloudflare Tunnel was
    replaced by the outbound Supabase Realtime subscriber.

    BotConfig MUST NOT carry ``webhook_secret``/``webhook_host``/
    ``webhook_port`` fields, and the module MUST NOT export
    ``WEBHOOK_DEFAULT_HOST``/``WEBHOOK_DEFAULT_PORT``.  Leaving them behind
    invites silent re-coupling to a removed capability.
    """

    def test_botconfig_has_no_webhook_fields(self) -> None:
        """No webhook_secret / webhook_host / webhook_port dataclass fields."""
        import dataclasses

        field_names = {f.name for f in dataclasses.fields(BotConfig)}
        assert "webhook_secret" not in field_names
        assert "webhook_host" not in field_names
        assert "webhook_port" not in field_names

    def test_botconfig_rejects_webhook_kwargs(self) -> None:
        """Constructing BotConfig with webhook kwargs MUST raise TypeError."""
        with pytest.raises(TypeError):
            BotConfig(webhook_secret="x")  # type: ignore[call-arg]

    def test_no_webhook_default_constants(self) -> None:
        """The module MUST NOT export WEBHOOK_DEFAULT_HOST/PORT."""
        import bot.config as cfg_mod

        assert not hasattr(cfg_mod, "WEBHOOK_DEFAULT_HOST")
        assert not hasattr(cfg_mod, "WEBHOOK_DEFAULT_PORT")
