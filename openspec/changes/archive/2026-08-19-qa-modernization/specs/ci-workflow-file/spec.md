...
(Previously: same triggers; cancel-in-progress already true)
...
(Previously: matrix applied to single qa-matrix job; now scoped to tests job)
...
(Reason: `actions/cache@v4` replaced by `astral-sh/setup-uv` native cache)
(Migration: `astral-sh/setup-uv` with cache enabled)

### Requirement: Blocking QA job covers bot and tests (cleanup-stability delta)

(Reason: superseded by three-job structure)
(Migration: quality + tests jobs together cover bot/ and tests/)

### Requirement: mypy/bandit/pip-audit steps (historical)

(Reason: mypy→ty, bandit→Ruff S, pip-audit→uv audit; steps deleted)
(Migration: ty in quality job; Ruff S already enforced; uv audit in quality job)
