...
- [ ] 2.1 Add `[tool.ty.environment] python-version="3.11"`; `[tool.ty.rules]` missing-type-argument=error, possibly-unresolved-reference=warn, unsound-return-statement=error, blanket-ignore-comment=error. `[tool.ty.analysis]` strict-literal/generic-narrowing=true. **Why**: Astral strict; rule names verified vs ty 0.0.18 registry. **Accept**: `ty check` reads config. **Evidence**: `uv run ty check --help`. **Dep**: Phase 1.
- [ ] 2.2 Add `[[tool.ty.overrides]] include=["bot/cogs/**"]` rules: invalid-argument-type=warn, possibly-missing-import=warn, possibly-unresolved-reference=warn (discord.py stub gaps; #3638 unsupported). `[[tool.ty.overrides]] include=["tests/**"]` possibly-unresolved-reference=warn, possibly-missing-attribute=warn. **Why**: cogs warn / rest error. **Accept**: cogs findings warn-tier. **Evidence**: `ty check bot/cogs/`. **Dep**: 2.1.
- [ ] 2.3 Delete `[tool.mypy]` + both `[[tool.mypy.overrides]]`. **Why**: ty replaces. **Accept**: no `[tool.mypy]` in pyproject. **Evidence**: `grep tool.mypy` empty. **Dep**: 2.2.
- [ ] 2.4 Makefile: `type` and `type-full` → `uv run ty check bot/ tests/`. **Why**: target migration. **Accept**: `make type` runs ty. **Evidence**: `make type`. **Dep**: 2.3.
- [ ] 2.5 ci.yml: `mypy` step → `ty check bot/ tests/`. **Why**: CI gate. **Accept**: no mypy step. **Evidence**: workflow YAML. **Dep**: 2.3.
- [ ] 2.6 Run baseline `uv run ty check bot/ tests/`; record actual finding count vs 28 target. **Why**: PR2 open question — measure before PR4. **Accept**: count documented in PR body. **Evidence**: ty output captured. **Dep**: 2.2.
- [ ] 2.7 RED→GREEN: write failing test asserting ty error diagnostic blocks (stage a typed violation in a scratch module under tests/); then confirm `ty check` exits non-zero. **Why**: strict TDD for blocking behavior. **Accept**: non-zero exit on error. **Evidence**: `ty check` exit≠0. **Dep**: 2.6.
- [ ] 2.8 Defer 28 (or actual count) findings WITHOUT `Any`/`cast` silencing; use `# ty: ignore[<rule>]` inline only where justified. **Why**: no broad suppression. **Accept**: deferred list in PR body. **Evidence**: `ty check` exit 0 on bot/ tests/. **Dep**: 2.7.
